// ============================================================
// محلّل جريدة أم القرى — يفصل القرارات/المراسيم/اللوائح عن الأخبار
// ويستخرج بيانات الربط. النصّ الحرفيّ (raw_text) شريحة من الأصل بلا تعديل.
// ============================================================

const AR_DIGITS = "٠١٢٣٤٥٦٧٨٩";

/** يحوّل الأرقام العربيّة-الهنديّة إلى ASCII — للمطابقة والاستخراج فقط، لا للتخزين. */
export function toAsciiDigits(s: string): string {
  return s.replace(/[٠-٩]/g, (d) => String(AR_DIGITS.indexOf(d)));
}

const D = "[٠-٩0-9]"; // صفّ أرقام (عربيّ أو لاتينيّ)

// مراسي الأحداث: رؤوس القرارات/المراسيم كما تُطبع في الجريدة
const ANCHORS: { re: RegExp; type: string }[] = [
  { re: new RegExp(`مرسوم\\s+ملكي\\s+رقم\\s*\\(?\\s*م\\s*/?\\s*${D}+\\s*\\)?`, "g"), type: "royal_decree" },
  { re: new RegExp(`قرار\\s+مجلس\\s+الوزراء\\s+رقم\\s*\\(?\\s*${D}+\\s*\\)?`, "g"), type: "cabinet_decision" },
  { re: new RegExp(`قرار\\s+وزير[^\\n]{0,60}?رقم\\s*\\(?\\s*${D}+\\s*\\)?`, "g"), type: "ministerial_decision" },
  { re: new RegExp(`قرار\\s+رقم\\s*\\(?\\s*${D}+\\s*\\)?`, "g"), type: "cabinet_decision" },
];

export type ParsedEvent = {
  event_type: string;
  instrument_number: string | null; // رقم هذا القرار/المرسوم
  authority: string | null;
  event_date_hijri: string | null;
  event_sort: number | null;
  affected_articles: string | null;
  raw_text: string; // شريحة حرفيّة من النصّ المُستخرَج
  // بيانات الربط بالنظام الأمّ:
  parent_reference: string | null; // مثل م/51
  parent_title: string | null; // اسم النظام/اللائحة المُستنتَج
  instrument_kind: string;
  match_keys: string[];
};

export type ParseResult = {
  regulatory_section_found: boolean;
  events: ParsedEvent[];
};

/** يحوّل تاريخًا هجريًّا (سنة/شهر/يوم) إلى رقم YYYYMMDD للفرز. */
export function hijriToSort(dateStr: string | null): number | null {
  if (!dateStr) return null;
  const ascii = toAsciiDigits(dateStr);
  const m = ascii.match(/(\d{3,4})\s*\/\s*(\d{1,2})\s*\/\s*(\d{1,2})/);
  if (!m) return null;
  const y = parseInt(m[1], 10);
  const mo = parseInt(m[2], 10);
  const d = parseInt(m[3], 10);
  if (!y || !mo || !d) return null;
  return y * 10000 + mo * 100 + d;
}

/** يستخرج أوّل تاريخ هجريّ ظاهر قرب رأس الحدث. */
function extractHijriDate(seg: string): string | null {
  const m = seg.match(new RegExp(`وتاريخ\\s*(${D}{3,4}\\s*/\\s*${D}{1,2}\\s*/\\s*${D}{1,2}\\s*هـ?)`));
  if (m) return m[1].trim();
  const m2 = seg.match(new RegExp(`(${D}{3,4}\\s*/\\s*${D}{1,2}\\s*/\\s*${D}{1,2})\\s*هـ`));
  return m2 ? m2[1].trim() + "هـ" : null;
}

/** رقم هذا القرار/المرسوم من رأسه. */
function extractOwnNumber(header: string): string | null {
  const royal = header.match(new RegExp(`رقم\\s*\\(?\\s*م\\s*/?\\s*(${D}+)`));
  if (royal) return "م/" + toAsciiDigits(royal[1]);
  const num = header.match(new RegExp(`رقم\\s*\\(?\\s*(${D}+)`));
  return num ? toAsciiDigits(num[1]) : null;
}

/** المرجع النظاميّ للنظام الأمّ (البصمة الأقوى للربط). */
function extractParentReference(seg: string): string | null {
  // "الصادر بالمرسوم الملكي رقم (م/51)" أو "الصادر بقرار مجلس الوزراء رقم (296)"
  const royal = seg.match(new RegExp(`(?:الصادر|الصادرة)[^\\n]{0,40}?بالمر?سوم\\s+الملكي\\s+رقم\\s*\\(?\\s*م\\s*/?\\s*(${D}+)`));
  if (royal) return "م/" + toAsciiDigits(royal[1]);
  const cab = seg.match(new RegExp(`(?:الصادر|الصادرة)[^\\n]{0,40}?بقرار\\s+مجلس\\s+الوزراء\\s+رقم\\s*\\(?\\s*(${D}+)`));
  if (cab) return toAsciiDigits(cab[1]);
  const min = seg.match(new RegExp(`(?:الصادر|الصادرة)[^\\n]{0,50}?بالقرار\\s+الوزاري\\s+رقم\\s*\\(?\\s*(${D}+)`));
  if (min) return toAsciiDigits(min[1]);
  return null;
}

/** اسم النظام/اللائحة الأمّ المُستنتَج (للعرض ومفتاح ربط ثانويّ). */
function extractParentTitle(seg: string): { title: string | null; kind: string } {
  // اللائحة التنفيذية لنظام ...
  let m = seg.match(/اللائحة\s+التنفيذية\s+لنظام\s+([^\n،.]{3,80}?)(?:[،.]|الصادر|الصادرة|\n|$)/);
  if (m) return { title: "اللائحة التنفيذية لنظام " + m[1].trim(), kind: "executive_regulation" };
  // تعديل نظام ... / على نظام ...
  m = seg.match(/(?:تعديل|على)\s+نظام\s+([^\n،.]{3,80}?)(?:[،.]|الصادر|،|\n|المُوافق|على النحو|$)/);
  if (m) return { title: "نظام " + m[1].trim(), kind: "system" };
  // قواعد ... / الترتيب التنظيمي ...
  m = seg.match(/(قواعد\s+[^\n،.]{3,80}?|الترتيب\s+التنظيمي\s+[^\n،.]{3,60}?)(?:[،.]|الصادر|الصادرة|\n|$)/);
  if (m) return { title: m[1].trim(), kind: "rules" };
  // الاتفاقية ...
  m = seg.match(/(الاتفاقية\s+[^\n،.]{3,80}?)(?:[،.]|الصادر|المُوافق|\n|$)/);
  if (m) return { title: m[1].trim(), kind: "agreement" };
  return { title: null, kind: "system" };
}

/** الجهة المُصدِرة. */
function extractAuthority(header: string, seg: string): string | null {
  if (/قرار\s+مجلس\s+الوزراء|إن\s+مجلس\s+الوزراء/.test(header + seg)) return "مجلس الوزراء";
  if (/مرسوم\s+ملكي/.test(header)) return "مرسوم ملكيّ";
  const min = seg.match(/قرار\s+(وزير\s+[^\n،(]{2,30}?)\s*رقم/);
  if (min) return min[1].trim();
  if (/البنك\s+المركزي/.test(seg)) return "البنك المركزي السعودي";
  if (/النيابة\s+العامة/.test(seg)) return "النيابة العامة";
  return null;
}

/** المواد المتأثّرة (أرقام المواد المذكورة في التعديل). */
function extractAffectedArticles(seg: string): string | null {
  const matches = [...seg.matchAll(new RegExp(`المادة\\s*\\(?\\s*(${D}{1,3})\\s*\\)?`, "g"))];
  const nums = [...new Set(matches.map((m) => toAsciiDigits(m[1])))].slice(0, 25);
  return nums.length > 0 ? nums.join("، ") : null;
}

/** نوع الحدث الدقيق من دلالة النصّ. */
function refineEventType(anchorType: string, seg: string): string {
  const head = seg.slice(0, 400);
  if (/إلغاء/.test(head) && !/إلغاء\s+الفقرة|إلغاء\s+المادة/.test(head)) return "repealed";
  if (/تعديل/.test(head)) return "amended";
  if (/اللائحة\s+التنفيذية/.test(head)) return "executive_regulation";
  if (/قواعد\s+وإجراءات|^قواعد|قواعد\s+تنظيم/.test(head)) return "rules";
  return anchorType;
}

/** يجد بداية القسم النظاميّ (يتجاوز الأخبار). */
function findRegulatoryStart(text: string): number {
  // أوّل مرسى قرار/مرسوم قويّ
  let earliest = -1;
  for (const a of ANCHORS) {
    a.re.lastIndex = 0;
    const m = a.re.exec(text);
    if (m && (earliest === -1 || m.index < earliest)) earliest = m.index;
  }
  return earliest;
}

/**
 * يحلّل النصّ الكامل لعدد إلى أحداث نظاميّة.
 * raw_text لكلّ حدث = شريحة حرفيّة من النصّ المُستخرَج (بلا تعديل).
 */
export function parseIssue(fullText: string): ParseResult {
  const start = findRegulatoryStart(fullText);
  if (start === -1) return { regulatory_section_found: false, events: [] };

  const region = fullText.slice(start);

  // اجمع كلّ مواضع المراسي وأنواعها
  type Hit = { index: number; type: string; len: number };
  const hits: Hit[] = [];
  for (const a of ANCHORS) {
    a.re.lastIndex = 0;
    let m: RegExpExecArray | null;
    while ((m = a.re.exec(region)) !== null) {
      hits.push({ index: m.index, type: a.type, len: m[0].length });
    }
  }
  if (hits.length === 0) return { regulatory_section_found: false, events: [] };

  // رتّب وأزِل المتداخلة القريبة جدًّا (نفس الرأس قد يُلتقط بأكثر من نمط)
  hits.sort((x, y) => x.index - y.index);
  const dedup: Hit[] = [];
  for (const h of hits) {
    const prev = dedup[dedup.length - 1];
    if (prev && h.index - prev.index < 25) continue; // نفس المرسى تقريبًا
    dedup.push(h);
  }

  const events: ParsedEvent[] = [];
  for (let i = 0; i < dedup.length; i++) {
    const segStart = dedup[i].index;
    const segEnd = i + 1 < dedup.length ? dedup[i + 1].index : region.length;
    const raw = region.slice(segStart, segEnd).trim();
    if (raw.length < 60) continue; // مقطع قصير جدًّا — تجاهل

    const header = raw.slice(0, 200);
    const parentRef = extractParentReference(raw);
    const { title: parentTitle, kind } = extractParentTitle(raw);
    const dateH = extractHijriDate(raw);

    const matchKeys: string[] = [];
    if (parentRef) matchKeys.push(parentRef);
    if (parentTitle) matchKeys.push(parentTitle.replace(/\s+/g, " ").trim());

    events.push({
      event_type: refineEventType(dedup[i].type, raw),
      instrument_number: extractOwnNumber(header),
      authority: extractAuthority(header, raw),
      event_date_hijri: dateH,
      event_sort: hijriToSort(dateH),
      affected_articles: extractAffectedArticles(raw),
      raw_text: raw, // ★ حرفيّ
      parent_reference: parentRef,
      parent_title: parentTitle,
      instrument_kind: kind,
      match_keys: matchKeys,
    });
  }

  return { regulatory_section_found: true, events };
}
